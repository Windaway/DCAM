...
class D646AUTO(nn.Module):
    def __init__(self):
        super(D646AUTO, self).__init__()
        self.backbone = resnet.ResNet50_MOD3AT(os=16)
        self.encoder = MSEncoder(output_stride=16)
        self.pred1=nn.Sequential(nn.Conv2d(384,256,1,1,0),nn.ReLU(),nn.Conv2d(256,128,1,1,0),nn.ReLU(),nn.Dropout(0.2),nn.Conv2d(128,3,1,1,0))
        ...
        self.predg1=nn.Sequential(nn.Conv2d(128,128,1,1,0),nn.ReLU(),nn.Conv2d(128,3,1,1,0))
        self.predg2=nn.Sequential(nn.Conv2d(128,128,1,1,0),nn.ReLU(),nn.Conv2d(128,3,1,1,0))
        ...
        self.seg_o2=nn.Sequential(nn.Conv2d(128+64,96,3,1,1,bias=True),nn.ReLU(),    nn.Dropout(0.1),
        ...
        self.convo=nn.Sequential(nn.Conv2d(64+3+32,24,3,1,1,bias=True),nn.PReLU(24),nn.Conv2d(24,16,3,1,1,bias=True),nn.PReLU(16),nn.Conv2d(16,1,3,1,1,bias=True))
        self.down=downs()
    ...
    def forward(self, x):
        ...
        trio=self.seg_o2(feats)
        feats=self.conv2(feats)
        feats=self.up(feats)
        feats=torch.cat([feats,img,features[-6]],1)
        ...
        return pred1,predg1,predg2,trio,alpha
